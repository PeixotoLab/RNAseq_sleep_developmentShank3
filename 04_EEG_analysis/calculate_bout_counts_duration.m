function [Avg_bout_counts, Avg_bout_durations] = calculate_bout_counts_duration(options)
%
% USAGE: [Avg_bout_counts, Avg_bout_durations] = calculate_bout_counts_duration(Scores,Sex,WTorMut,BLorSD,bout_mins,TimeFrame)
%
%
...
for i=1:length(Scores)
    ...
    if ~isempty(W_idx) 					% If there are some W runs
		...
		W_bout_durations  = W_bout_durations(W_bout_durations>=bout_mins.W);	% Keep only those bout durations that are longer than the minimum bout length
	else 
